...
void NNClassifier::rnn_feed_forward(
        DSCTree & tree,
        int root,
        unordered_map<int, Vec<double> > & rnn)
{
    // forward form the leaf node
...
void NNClassifier::rnn_back_propagate(
        ...
        int root,
        unordered_map<int, Vec<double> > & rnn,
        unordered_map<int, Vec<double> > & grad_rnn,
        Mat<double> & grad_Eb,
        Mat<double> & grad_Wv,
        Mat3<double> & grad_Wr)
{
    ...
    for (int i = 0; i < config.compose_embedding_size; ++i)
    {
        for (int j = 0; j < Eb.ncols(); ++j)
        {
            grad_Wv[i][j] += Wv_weight * grad_net[i] * Eb[tree.wordids[root]][j];
            grad_Eb[tree.wordids[root]][j] += Wv_weight * grad_net[i] * Wv[i][j];
        }
    }
...
Cost NNClassifier::thread_proc(vector<Sample> & chunk, size_t batch_size)
{
    ...
    for (size_t i = 0; i < chunk.size(); ++i)
    {
        ...
        DSCTree dsctree = chunk[i].get_dsctree();
        ...
        for (int j = 0; j < config.num_compose_tokens; ++j)
        {
            ...
            rnn_feed_forward(dsctree, tok_w_root, dscrnn);
            vec_dscrnn.push_back(dscrnn);
            ...
            for (size_t k = 0; k < active_units.size(); ++k)
            {
                ...
                {
                    hidden[node_index] +=
                        W1[node_index][offset+l] * dscrnn[tok_w_root][l];
                }
            }
        ...
        */
        ...
        for (int j = 0; j < config.num_compose_tokens; ++j)
        {
            ...
            // initialize grad_h_root
            Vec<double> grad_h_root(.0, config.compose_embedding_size);
            grad_dscrnn[tok_w_root] = grad_h_root;
            ...
            for (size_t k = 0; k < active_units.size(); ++k)
            {
                ...
                for (int l = 0; l < config.compose_embedding_size; ++l)
                {
                    grad_W1[node_index][offset+l] += grad_hidden[node_index] * dscrnn[tok_w_root][l];
                    grad_dscrnn[tok_w_root][l] += grad_hidden[node_index] * W1[node_index][offset+l];
                }
            }

            rnn_back_propagate(dsctree, tok_w_root, dscrnn, grad_dscrnn, grad_Eb, grad_Wv, grad_Wr);
...
double NNClassifier::compute_cost()
{
    ...
    for (size_t i = 0; i < samples.size(); ++i)
    {
        ...
        DSCTree dsctree = samples[i].get_dsctree();
        ...
        vector<double>& scores)
{
    ...
    for (int j = 0; j < config.num_compose_tokens; ++j)
    {
        ...
        for (int k = 0; k < config.hidden_size; ++k)
        {
            for (int l = 0; l < config.compose_embedding_size; ++l)
            {
                hidden[k] += W1[k][offset+l] * dscrnn[tok_w_root][l];
            }
        }
        offset += config.compose_embedding_size;
    }
