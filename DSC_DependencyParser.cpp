...
#include "DSC_DependencyParser.h"
#include "Util.h"
#include "Config.h"
#include "time.h"
        ...
        const char * premodel_file)
{
    ...
    // hard-code the composed token, (perhaps) can be unified.
    int W1_ncol = config.embedding_size * config.num_basic_tokens
                + config.compose_embedding_size * config.num_compose_tokens;
    if (config.use_distance)
    ...
    for (int i = 0; i < Wr.dim1(); ++i)
        for (int j = 0; j < Wr.dim2(); ++j)
            for (int l = 0; l < Wr.dim3(); ++l)
                Wr[i][j][l] = (Util::rand_double() * 2 - 1) * Wr_init_range;
    ...
    if (premodel_file[0] != 0)
    {
        ...
        // embedding size of composition units
        getline(input, s); int Ecomp_size = to_int(split_by_sep(s, "=")[1]);
        getline(input, s); int h_size = to_int(split_by_sep(s, "=")[1]);
        getline(input, s); int n_basic_tokens = to_int(split_by_sep(s, "=")[1]);
        // number of composition units
        getline(input, s); int n_compose_tokens = to_int(split_by_sep(s, "=")[1]);
        ...
        assert (n_compose_tokens == config.num_compose_tokens);
        assert (Ecomp_size == config.compose_embedding_size);
        ...
        { // n_labels
            ...
            { // dim3
                getline(input, s);
                sep = split(s);
                for (int j = 0; j < Wr.dim2(); ++j) // dim2
                    Wr[i][j][l] = to_double_sci(sep[j]);
            }
        }
    ...
    classifier = new NNClassifier(config, dataset, Eb, Ed, Ev, Ec, W1, b1, W2, Wv, Wr, pre_computed_ids);
}
    ...
    for (size_t i = 0; i < sents.size(); ++i)
    {
        ...
        if (trees[i].is_projective())
        {
            ...
            while (!system->is_terminal(c))
            {
                ...
                // a trick for saving space
                //  - only save subtrees rooted from S0/S1
                update_dsctree_standard(dsctree, c, oracle);
...
void DependencyParser::save_model(const char * filename)
{
    ...
    Mat<double>& Wv = classifier->get_Wv();
    Mat3<double>& Wr = classifier->get_Wr();
    ...
    output << "dict="                   << known_words.size()               << "\n"
           ...
           << "composeembeddingsize="   << config.compose_embedding_size    << "\n"
           << "hiddensize="             << b1.size()                        << "\n"
           << "basictokens="            << config.num_basic_tokens          << "\n"
           << "composetokens="          << config.num_compose_tokens        << "\n"
...
void DependencyParser::update_dsctree_standard(
        DSCTree & tree,
        Configuration & c,
        const string & t)
{
    // update DSCTree using arc-standard algorithm
    ...
    // tree.add_word(w2, get_word_id(c.get_word(w2)));
}
        ...
        DependencyTree& tree)
{
    ...
    while (!system->is_terminal(c))
    {
        ...
        vector<int> features = get_features(c);
        classifier->compute_scores(features, dsctree, scores);
        ...
        // cerr << opt_trans << "->";
        update_dsctree_standard(dsctree, c, opt_trans);
...
void DependencyParser::load_model(const char * filename, bool re_precompute)
{
    ...
    if (!config.delexicalized)
        for (int i = 0; i < n_dict; ++i)
        {
            ...
            assert (sep.size() == (unsigned)Eb_size + 1);
            for (int j = 0; j < Eb_size; ++j)
                Eb[index][j] = to_double_sci(sep[j+1]);
            index += 1;
        }
    ...
    if (config.use_distance)
        for (int i = 0; i < n_dist; ++i)
        {
            ...
            assert (sep.size() == (unsigned)Ed_size + 1);
            for (int j = 0; j < Ed_size; ++j)
                Ed[index][j] = to_double_sci(sep[j+1]);
            index += 1;
        }
    ...
    if (config.use_valency)
        for (int i = 0; i < n_valency; ++i)
        {
            ...
            assert (sep.size() == (unsigned)Ev_size + 1);
            for (int j = 0; j < Ev_size; ++j)
                Ev[index][j] = to_double_sci(sep[j+1]);
            index += 1;
        }
    ...
    if (config.use_cluster)
        for (int i = 0; i < n_cluster; ++i)
        {
            ...
            assert (sep.size() == (unsigned)Ec_size + 1);
            for (int j = 0; j < Ec_size; ++j)
                Ec[index][j] = to_double_sci(sep[j+1]);
            index += 1;
        }
    ...
    for (int j = 0; j < W1.ncols(); ++j)
    {
        ...
        assert (sep.size() == (unsigned)h_size);
        for (int i = 0; i < W1.nrows(); ++i)
            W1[i][j] = to_double_sci(sep[i]);
    }
    ...
    for (int j = 0; j < W2.ncols(); ++j)
    {
        getline(input, s);
        vector<string> sep = split(s);
        assert (sep.size() == (unsigned)n_actions);
        for (int i = 0; i < W2.nrows(); ++i)
            W2[i][j] = to_double_sci(sep[i]);
    }

    Mat<double> Wv(Ecomp_size, Eb_size);
    for (int j = 0; j < Wv.ncols(); ++j)
    {
        getline(input, s);
        sep = split(s);
    ...
    input.close();
    if (re_precompute)
        classifier = new NNClassifier(config, Eb, Ed, Ev, Ec, W1, b1, W2, Wv, Wr, vector<int>());
    else
        classifier = new NNClassifier(config, Eb, Ed, Ev, Ec, W1, b1, W2, Wv, Wr, pre_computed_ids);
